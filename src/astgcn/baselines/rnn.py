...
class RNNBaseline(nn.Module):
    ...
    模型把每个节点的 recent 时间序列视为一个样本，使用同一套 RNN 参数预测
    未来 pred_len 个时间步。输入输出接口与 GRUBaseline / LSTMBaseline 保持一致。
    """
    def __init__(
        self,
        in_channels: int,
        ...
        :param hidden_size: RNN 隐状态维度。
        :param pred_len: 预测长度。
        :param num_layers: RNN 层数。
        :param dropout: RNN 层间 dropout，单层时自动置为 0。
        :param nonlinearity: RNN 激活函数，可选 "tanh" 或 "relu"。
        """
        super().__init__()
        ...
        if nonlinearity not in {"tanh", "relu"}:
            raise ValueError('nonlinearity 只能是 "tanh" 或 "relu"')
    ...
    def forward(self, recent: torch.Tensor, *_args, **_kwargs) -> torch.Tensor:
        ...
        x = recent.permute(0, 1, 3, 2).contiguous()
        x = x.reshape(batch_size * num_nodes, num_timesteps, self.in_channels)
